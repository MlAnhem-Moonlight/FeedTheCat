...
namespace UnityEngine.UI.Extensions
{
    public class ScrollSnapBase : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IScrollSnap, IPointerClickHandler
    {
        ...
        [Tooltip("Toggle prefab to dynamically generate pagination toggles. (optional)\nIf assigned, toggles will be auto-generated when content changes")]
        [SerializeField]
        public GameObject PaginationTogglePrefab;
        ...
        /// <summary>
        /// Dynamically generates pagination toggles from a prefab to match the page count
        /// </summary>
        public void GeneratePaginationToggles()
        {
            if (PaginationTogglePrefab == null || Pagination == null)
                return;

            // Get the number of pages
            int pageCount = _screensContainer != null ? _screensContainer.childCount : 0;
            if (pageCount == 0)
                return;

            // Clear existing toggles
            foreach (Transform child in Pagination.transform)
            {
            ...
            {
                GameObject toggleObj = Instantiate(PaginationTogglePrefab, Pagination.transform, false);
                toggleObj.name = $"Toggle_{i}";

                // Ensure the Toggle component exists
                Toggle toggle = toggleObj.GetComponent<Toggle>();
                if (toggle == null)
                    toggle = toggleObj.GetComponentInChildren<Toggle>();

                if (toggle != null)
                {
                    // First toggle is on, others are off
                    toggle.isOn = (i == 0);
                    toggle.group = GetComponent<ToggleGroup>() ?? Pagination.GetComponent<ToggleGroup>();
                }
            }

            // If PaginationManager exists, notify it to reset children
            var paginationManager = Pagination.GetComponent<PaginationManager>();
