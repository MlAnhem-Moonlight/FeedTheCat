    ...
    public class CharmItem4Radius : ItemBase
    {
        ...
        protected override void ExecuteEffect(GridCell target)
        {
            ...
            if (statusSystem != null)
            {
                ...
                Debug.Log($"CharmItem4Radius.ExecuteEffect: Applied Charm to {affectedNPCs.Count} NPCs for {itemData.EffectDuration} turns");
            }
            else
            {
        ...
        private void CommandCharmMovement(NPCMover npc, GridCell targetCell)
        {
            ...
            Debug.Log($"CharmItem4Radius.CommandCharmMovement: Commanding '{npc.gameObject.name}' to move toward {targetCell.gameObject.name}");
